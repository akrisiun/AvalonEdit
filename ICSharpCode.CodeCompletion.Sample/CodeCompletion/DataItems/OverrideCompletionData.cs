...
namespace ICSharpCode.CodeCompletion.DataItems
{
    ...
    internal class OverrideCompletionData : EntityCompletionData
    {
            ...
            EventArgs insertionRequestEventArgs)
        {
            ...
            {
                // ICSharpCode.AvalonEdit.Document.
                ISegment docCompletionSegment = null;

                base.Complete(textArea, docCompletionSegment, insertionRequestEventArgs);
                return;
            }
            ...
            formatter.AddFormattingRegion(new DomRegion(
                doc.GetLocation(declarationBegin), 
                doc.GetLocation(declarationBegin + newText.Length)));
            
            var syntaxTree = new CSharpParser().Parse(doc);
            formatter.AnalyzeFormatting(doc, syntaxTree).ApplyChanges();
        }
